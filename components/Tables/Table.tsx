...
const Table = () => {
  ...
  const { data }: any = useQuery(QUERY_KEY.userlist, () =>
    request('http://localhost:4200/graphql', GET_USERS),
  );
  console.log(data);
  ...
  const currentUsers = data?.getAllusers?.slice(indexOfFirstUser, indexOfLastUser) || [];

  const handlePageChange = (page: number) => {
    setCurrentPage(page);
  };
  const queryClient = useQueryClient();
  const deleteUser = useMutation<void, { id: string }>(
    async ({ id }: any) => {
      await request("http://localhost:4200/graphql", DELETE_USER, { id });
    },
    {
      onSuccess: async () => {
      ...
      onError: () => {
        toastNotification("Delete failed", "error", 3000);
      },
    }
  );
  ...
  const userSetting = async (id: string) => {
    const selectedUser = data?.getAllusers?.find((user: IUser) => user._id === id);
    if (selectedUser) {
      router.push({
        pathname: "/settings",
        query: { id: selectedUser._id, name: selectedUser.name, email: selectedUser.email },
      });
    } else {
      return;
    }
    ...
    <div className="rounded-sm border px-5 pt-6 pb-2.5 shadow-default border-strokedark bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full ">
              ...
              <tr key={key}>
                ...
                <td className="border-b  py-5 px-4 border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    ...
                    onClick={() => userSetting(subAllUser._id)}
                    >
                      <svg
                    ...
                    <button className="hover:text-primary"
                      onClick={() => handleOnClick(subAllUser._id)}
                    >
                      <svg
                        className="fill-current"
                        width="18"
        ...
        <Pagination
          ...
          total={data?.getAllusers?.length || 0}
